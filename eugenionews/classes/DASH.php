...
class DASH
{
    ...
    public static function listNoticia($table, $start = null, $limit = null)
    {
        ...
        if ($start === null && $limit === null) {
            $sql = SQLCN::connectDB()->prepare("(SELECT * FROM `$table` WHERE order_id = 1)UNION ALL (SELECT * FROM `$table` WHERE order_id = 0) ORDER BY CASE WHEN order_id = 1 THEN 0 ELSE 1 END, id DESC");
        } else {
            $sql = SQLCN::connectDB()->prepare("(SELECT * FROM `$table` WHERE order_id = 1)UNION ALL (SELECT * FROM `$table` WHERE order_id = 0) ORDER BY CASE WHEN order_id = 1 THEN 0 ELSE 1 END, id DESC LIMIT :start, :limit;
");
            $sql->bindParam(':start', $start, PDO::PARAM_INT);
            $sql->bindParam(':limit', $limit, PDO::PARAM_INT);
        }
    ...
    {
        $sql = SQLCN::connectDB()->prepare("UPDATE `tb_painel_admin.noticias` SET order_id = 0 WHERE order_id = 1");
        $sql->execute();
    }
}
